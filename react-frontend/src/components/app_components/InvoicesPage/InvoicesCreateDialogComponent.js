...
const InvoicesCreateDialogComponent = (props) => {
    ...
    useEffect(() => {
        ...
        if (!_.isEmpty(props?.entity)) {
            init = initilization({ ...props?.entity, ...init }, [companyId], setError);
        }
        set_entity({...init});
        setError({});
    }, [props.show]);
    ...
    const validate = () => {
        ...
        const error = {};
            ...
            if (_.isEmpty(_entity?.item)) {
                error["item"] = `Item field is required`;
                ret = false;
            }
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            companyId: _entity?.companyId?._id,item: _entity?.item,subTotal: _entity?.subTotal,total: _entity?.total,
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <InputText id="item" className="w-full mb-3 p-inputtext-sm" value={_entity?.item} onChange={(e) => setValByKey("item", e.target.value)}  required  />
            </span>
            <small className="p-error">
